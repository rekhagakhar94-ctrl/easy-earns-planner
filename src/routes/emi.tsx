  ...
  return (
    <AppShell>
      ...
      <div className="mt-6 space-y-5">
        ...
        <Card>
          <Field label="Loan Amount" hint={<NumberPill value={amount} onChange={setAmount} prefix={symbol} min={0} max={100000000} step={1000} placeholder="0" />}>
            <Slider value={Math.min(numOr(amount), 1000000)} min={1000} max={1000000} step={1000} onChange={setAmount} />
          </Field>
        </Card>
        <Card>
          <Field label="Interest Rate (p.a)" hint={<NumberPill value={rate} onChange={setRate} suffix="%" min={0} max={50} step={0.1} placeholder="0" />}>
            <Slider value={Math.min(numOr(rate), 20)} min={1} max={20} step={0.1} onChange={setRate} />
          </Field>
        </Card>
        <Card>
          <Field label="Loan Tenure" hint={<NumberPill value={years} onChange={setYears} suffix="Yrs" min={1} max={50} placeholder="0" />}>
